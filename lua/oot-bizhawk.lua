...
console.log('OoT initialization complete');

-- WebSocket server for interfacing with AP client
...
local function onMessage(msg)
    connection:send(string.format("Got message: %s", msg))
end

local function connectToAPServer()
    if stopped then
        return nil
    end
    ...
    if not connected then
        ...
        if (returnCode == nil) then
            console.log("Error while connecting: " .. errorMessage)
            stopped = true
            connected = false
            console.log("Please restart the LUA script to connect to the AP Client.")
            return
        end
    ...
    if status == 'closed' then
        print('Connection to AP Client is closed.')
        connection:close()
        connected = false
        return
    end
